...
void mdns_init()
{
	...
	obs_log(LOG_INFO, "mDNS loaded successfully (version %s)",
		PLUGIN_VERSION);
	mdns_rx_socket = socket(AF_INET, SOCK_DGRAM, 0);
	if (setsockopt(mdns_rx_socket, SOL_SOCKET, SO_REUSEPORT, &(int){1},
		       sizeof(int)) < 0) {
		obs_log(LOG_ERROR, "Failed to set mDNS receive socket options");
		goto cleanup;
	}
	if (setsockopt(mdns_rx_socket, IPPROTO_IP, IP_PKTINFO, &(int){1},
		       sizeof(int)) < 0) {
		obs_log(LOG_ERROR,
			"Failed to set mDNS receive socket IP_PKTINFO options");
		goto cleanup;
	}
	if (bind(mdns_rx_socket, (struct sockaddr *)&mdns_rx_addr,
		 sizeof(mdns_rx_addr)) < 0) {
...
void *_mdns_listen()
{
	...
	if ((errno = pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS,
					   NULL)) != 0) {
		obs_log(LOG_ERROR,
			"Failed to set mDNS listen thread cancel type");
	...
	while (1) {
		...
		obs_log(LOG_DEBUG, "Received mDNS packet of length %d from %s",
			len,
			inet_ntoa(((struct sockaddr_in *)header->msg_name)
					  ->sin_addr));
		mdns_packet_header *parsed_packet =
			parse_packet_header(&buffer[0], len);
		if (parsed_packet->qr == 1) {
			obs_log(LOG_INFO, "Ignoring response packet");
			continue;
		}
		int offset = 12;
		parsed_mdns_question **questions = parse_dns_questions(
			&buffer[0], &offset, parsed_packet->qdcount, len);
		for (int i = 0; i < parsed_packet->qdcount; i++) {
			obs_log(LOG_INFO, "Question %d: %s: %d", i,
				questions[i]->name, questions[i]->type);
			obs_log(LOG_INFO, "Is it for us %s? %d",
				OBS_EASYJOIN_HOST_NAME,
				strncmp((char *)questions[i]->name,
					(char *)OBS_EASYJOIN_HOST_NAME,
					questions[i]->name_len));
			if (questions[i]->type == RECORD_A &&
			    strncmp((char *)questions[i]->name,
				    (char *)OBS_EASYJOIN_HOST_NAME,
				    questions[i]->name_len) == 0) {
				dns_answer *answer = construct_A_answer(
					OBS_EASYJOIN_HOST_NAME,
					((struct sockaddr_in *)header->msg_name),
					120);
				int pkt_len = 0;
				unsigned char *data = to_datagram(
					parsed_packet, /* questions, */ &answer,
					1, &pkt_len);
				int send_len = 0;
				if (questions[i]->unicast_response) {
					...
					if ((send_len = sendto(
					    ...
					    -1) {
						obs_log(LOG_ERROR,
							"Failed to send mDNS response %d",
							send_len);
					}
				} else {
...
mdns_packet_header *parse_packet_header(unsigned char *packet, int length)
{
	...
	parsed_packet->opcode = (ntohs(*(unsigned short *)&packet[2]) >> 11) &
				0xf;
...
parsed_mdns_question **parse_dns_questions(unsigned char *packet, int *offset,
					   int count, int length)
{
	int start = *offset;
	parsed_mdns_question **questions =
		malloc(sizeof(parsed_mdns_question *) * count);
	for (int i = 0; i < count && start + i < length; i++) {
		parsed_mdns_question *question =
			malloc(sizeof(parsed_mdns_question));
		...
		while (packet[*offset] != 0) {
			...
			if (label_len & 0xc0) {
				...
				*offset = start +
				...
				(*offset)++;
				question->name = realloc(question->name,
							 question->name_len +
								 label_len + 1);
				memcpy(&question->name[question->name_len],
		...
		question->unicast_response =
			ntohs(*(unsigned short *)&packet[*offset]) >> 15;
		(*offset) += 2;
...
dns_answer *construct_A_answer(char *name, sockaddr_in *addr, unsigned int ttl)
{
	...
	while (name_offset < strnlen((char *)name, 64)) {
		int label_len = 0;
		while (name[label_offset + label_len] != '.' &&
		       name[label_offset + label_len] != '\0') {
			label_len++;
		}
		if (label_len == 0) {
			break;
		...
		memcpy(&answer->name[label_offset + 1], &name[label_offset],
		       label_len);
...
dns_answer *construct_AAAA_answer(char *name, sockaddr_in6 *addr,
				  unsigned int ttl)
{
	dns_answer *answer = malloc(sizeof(dns_answer));
	answer->name = malloc(strnlen((const char *)name, 64) + 1);
	...
	while (name_offset < strnlen((char *)name, 64)) {
		...
		memcpy(&answer->name[name_offset], &name[name_offset],
		       label_len);
...
unsigned char *
to_datagram(mdns_packet_header *packet,
	    /* TODO parsed_mdns_question** _questions,*/ dns_answer **answers,
	    int answer_count, int *length)
{
	...
	*(unsigned short *)&buffer[offset] = htons(packet->id);
	...
	packet->qr = 1;
	packet->aa = 1;
	*(unsigned short *)&buffer[offset] = htons(
		(packet->qr << 15) | (packet->opcode << 11) |
		(packet->aa << 10) | (packet->tc << 9) | (packet->rd << 8) |
		(packet->ra << 7) | (packet->z << 4) | packet->rcode);
	offset += 2;
