...
void main()
{
	unsigned char packet[] = {0xdb, 0x42, 0x01, 0x00, 0x00, 0x01, 0x00,
				  0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x77,
				  0x77, 0x77, 0x0c, 0x6e, 0x6f, 0x72, 0x74,
				  0x68, 0x65, 0x61, 0x73, 0x74, 0x65, 0x72,
				  0x6e, 0x03, 0x65, 0x64, 0x75, 0x00, 0x00,
				  0x01, 0x00, 0x01};
	mdns_packet_header *parsed_packet =
		parse_packet_header(packet, sizeof(packet));
	...
	parsed_mdns_question **questions =
		parse_dns_questions(packet, &offset, 1, sizeof(packet));
	assert(strncmp((char *)questions[0]->name, "www.northeastern.edu.",
		       questions[0]->name_len) == 0);
	// print each question
	...
	answers[0] = answer;
	int pkt_len = 0;
	unsigned char *data = to_datagram(parsed_packet,
					  /* questions,*/ answers, 1, &pkt_len);
